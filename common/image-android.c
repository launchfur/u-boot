...
struct vendor_boot_img_hdr *get_vendor_hdr_addr(void)
{
	...
	if (!already_vendor) {
		already_vendor = 1;
		sunxi_partition_get_info_byname("vendor_boot", &part_offset, &part_size);
		if (!part_offset) {
			pr_err("no %s partition is found\n", "vendor_boot");
			return 0;
		} else {
			sunxi_flash_read(part_offset, part_size, (char *)0x49000000);
		}
		vendor_hdr = (vendor_boot_img_hdr *)0x49000000;
	}
	return vendor_hdr;
}
			      ...
			      ulong *rd_data, ulong *rd_len)
{
	...
	if (hdr->unused >= 0x3) {
		...
		ramdisk_size = ALIGN(hdr_v3->ramdisk_size, page_size);
	}
	if (!ramdisk_size) {
		*rd_data = *rd_len = 0;
		return -1;
				...
				ulong *rd_data, ulong *rd_len)
{
	...
	if (vendor_hdr != NULL) {
		...
		*rd_len = ALIGN(vendor_hdr->vendor_ramdisk_size, vendor_hdr->page_size);
		debug("rd_data:0x%lx rd_len:0x%lx\n", *rd_data, *rd_len);
