...
static int sunxi_close_ce(void)
{
#ifdef CONFIG_MACH_SUN50IW10
	writel(readl(0x03001680) & (~(0x1<<31)) , 0x03001680);
	writel(readl(0x0300168c) & (~(0x1<<16 | 0x1)) , 0x0300168c);
#endif
	return 0;
}
...
static void __mute_key_jump_fel_lradc(void)
{
	int loop = 5;
	int value = -1;
	//init lradc(mute) key
	...
	for (; loop > 0; loop--) {
		...
		if (value >= 0) {
			...
			printf("detect usb first!\n");
			if (sunxi_usb_detect() != 0)
				break;   /* detect usb fail, ignore mute key, break to continue boot */
#endif
			rtc_set_fel_flag();
			reset_cpu(0);
		}
...
static void __mute_key_jump_fel_gpio(void)
{
	...
	int value = -1;
	//init gpio(mute) key
	gpio_direction_input(SUNXI_GPF(5));
	for (; loop > 0; loop--) {
		value = gpio_get_value(SUNXI_GPF(5));
		if (value == 0) {
			printf("get mute key input,now jump to fel\n\n");
#ifdef CONFIG_SUNXI_USB_DETECT
			printf("detect usb first!\n");
...
int mute_key_jump_fel(void)
{
	if (get_boot_work_mode() == WORK_MODE_BOOT) {
		int nodeoffset;
		char *status = NULL;
		char *mute_key_fel_type = NULL;
		nodeoffset = fdt_path_offset(working_fdt, "/soc/mute_key_fel");
		if (nodeoffset > 0) {
			fdt_getprop_string(working_fdt, nodeoffset, "status", &status);
			if (strcmp(status, "okay") == 0) {
				fdt_getprop_string(working_fdt, nodeoffset, "mute_key_fel_type", &mute_key_fel_type);
				printf("mute_key_fel_type : %s\n", mute_key_fel_type);
#ifdef CONFIG_SUNXI_LRADC_KEY
				if (strcmp(mute_key_fel_type, "lradc") == 0) {
					__mute_key_jump_fel_lradc();
				}
#endif
				if (strcmp(mute_key_fel_type, "gpio") == 0) {
