...
static void _show_layer_on_dev(void *layer_config, int sel, char is_show)
{
	...
	if (is_show) {
		...
		layer->info.screen_win.x = (layer->info.screen_win.width -layer->info.fb.size[0].width) / 2;
		layer->info.screen_win.y = 0;
                layer->info.screen_win.width = layer->info.fb.size[0].width;
		layer->info.screen_win.height = layer->info.fb.size[0].height;
