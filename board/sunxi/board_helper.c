...
static struct bootloader_control ab_message;
...
#define KEY_PHOTO_DT_PATH		"/soc/gpiokey/photo_key"
#define KEY_AI_DT_PATH			"/soc/gpiokey/enter_key"
#define key_info(fmt...) tick_printf(fmt)



int  check_photo_key_and_ai_key(void)
{
	int ret;
	user_gpio_set_t	gpio_photo;
	user_gpio_set_t	gpio_ai;
	__u32 gpio_photo_id;
	__u32 gpio_ai_id;
	int gpio_photo_value = 0;
	int gpio_ai_value = 0;

	/*for setup photo gpio*/
	ret = fdt_get_one_gpio(KEY_PHOTO_DT_PATH, "gpios", &gpio_photo);
	if (ret) {
		key_info("[photo_key] can't find photo_key config.\n");
		return 0;
	}
	gpio_photo.mul_sel = 0;
	gpio_photo_id = sunxi_gpio_request(&gpio_photo, 1);
	if (!gpio_photo_id) {
		pr_error("[key photo] gpio request fail!\n");
		return 0;
	...
	/*for setup ai gpio*/
	ret = fdt_get_one_gpio(KEY_AI_DT_PATH, "gpios", &gpio_ai);
	if (ret) {
		key_info("[photo_key] can't find photo_key config.\n");
		return 0;
	}
	gpio_ai.mul_sel = 0;
	gpio_ai_id = sunxi_gpio_request(&gpio_ai, 1);
	if (!gpio_ai_id) {
		pr_error("[key ai] gpio request fail!\n");
		return 0;
		/*gpio request fail,just return.*/
	}

	gpio_photo_value = gpio_read_one_pin_value(gpio_photo_id, 0);

	gpio_ai_value = gpio_read_one_pin_value(gpio_ai_id, 0);

	if(gpio_photo_value ==0 &&  gpio_ai_value==0)
		env_set("key_photo_ai","down_true");
	else
		env_set("key_photo_ai","down_false");
	key_info("photo read gpio_value = %d,ai key read gpio value = %d\n",gpio_photo_value,gpio_ai_value);
	return 0;
...
static int sunxi_get_bootcmd_from_misc(void)
{
	...
	if (!misc_offset) {
		...
		sunxi_flash_read(misc_offset + VENDOR_SPACE_OFFSET_IN_MISC/512, 1, (char *)misc_args);
		memcpy((char *)&ab_message, misc_args, sizeof(struct bootloader_control));
...
int sunxi_set_bootcmd(char *bootcmd)
{
	...
	case SUNXI_BOOT_RECOVERY_FLAG:
		if (ab_message.magic != BOOT_CTRL_MAGIC) {
...
int sunxi_update_bootcmd(void)
{
	...
	if (ab_message.magic == BOOT_CTRL_MAGIC) {
		memset(boot_commond, 0x0, 128);
		strncpy(boot_commond, env_get("boot_normal"), sizeof(boot_commond)-1);
		if (!strncmp(ab_message.slot_suffix, "_b", 2)) {
			sunxi_str_replace_all(boot_commond, "boot", "boot_b");
			env_set("slot_suffix", "_b");
		} else {
			sunxi_str_replace_all(boot_commond, "boot", "boot_a");
			env_set("slot_suffix", "_a");
		}
		env_set("boot_normal", boot_commond);
	}
