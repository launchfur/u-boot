...
void board_bootlogo_display(void)
{
	...
	//sunxi_bmp_display("bootlogo.bmp");//bat\\low_pwr.bmp
	/*
		BATTERY_RATIO_TOO_LOW_WITHOUT_DCIN;
		BATTERY_RATIO_ENOUGH;

	*/
	tick_printf("gd->pmu_runtime_chgcur = %d\n",gd->pmu_runtime_chgcur);
	
	if(gd->pmu_runtime_chgcur == BATTERY_RATIO_TOO_LOW_WITHOUT_DCIN)
...
int board_late_init(void)
{
	if (get_boot_work_mode() == WORK_MODE_BOOT) {
		...
		update_user_data();
		extern int update_user_data_ali(void);
		update_user_data_ali();
#endif
