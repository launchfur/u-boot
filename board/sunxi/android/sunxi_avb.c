...
int sunxi_avb_read_vbmeta_head(char *name, u8 *addr)
{
	u32 start_block;
	int ret     = 0;
	start_block = sunxi_partition_get_offset_byname_avb(name);
	if (!start_block) {
...
int sunxi_partition_get_offset_byname_avb(const char *part_name)
{
	char part_name_buffer[20] = { 0 };
	char *slot_suffix = env_get("slot_suffix");

	strcat(part_name_buffer, part_name);
	if (slot_suffix != NULL) {
	    strcat(part_name_buffer, slot_suffix);
	}
	return sunxi_partition_get_offset_byname(part_name_buffer);
}
				...
				char *sha1)
{
	...
	start_block	= sunxi_partition_get_offset_byname_avb("vbmeta");
	if (!start_block) {
	...
	start_block = sunxi_partition_get_offset_byname_avb("vbmeta_system");
	if (!start_block) {
		pr_msg("cant find part named vbmeta\n");
	} else {
	...
	start_block = sunxi_partition_get_offset_byname_avb("vbmeta_vendor");
	if (!start_block) {
		pr_msg("cant find part named vbmeta\n");
	} else {
		rbyte = sunxi_flash_read(start_block,
